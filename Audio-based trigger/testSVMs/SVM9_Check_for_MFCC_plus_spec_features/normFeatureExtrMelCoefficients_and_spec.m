...
function [featureVector,labels] = normFeatureExtrMelCoefficients_and_spec(audio,fs,audioname,vectorClass)
...
for i=1:numOfFrames  %for each frame
	...
	if(sum(abs(frame))>eps)
		...
		Features(6,i) = HR;
		Features(7,i) = F0;
		Features(8,i) = spectral_entropy(frameFFT);
	else
